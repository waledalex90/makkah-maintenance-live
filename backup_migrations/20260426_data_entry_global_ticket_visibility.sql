-- مدخل بيانات العمليات: رؤية كل البلاغات (مثل الإدارة) لاستعلامات الواجهة والإحصاءات
-- كان القيد السابق يعتمد على zone_profiles فقط، فيظهر 0 عند عدم تطابق كل المناطق.
...
as $$
  ...
  select
    case
      when (select role from me)::text in ('admin', 'project_manager', 'projects_director', 'data_entry') then true
